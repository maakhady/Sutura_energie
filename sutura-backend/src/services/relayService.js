...
const RASPBERRY_PI_URL =
  process.env.RASPBERRY_PI_URL || "http://192.168.1.57:2500";
...
exports.activerDesactiverRelay = async (appareil) => {
  try {
    ...
    if (
      !appareil ||
      !appareil.relay_ID ||
      typeof appareil.relay_ID !== "number"
    ) {
      throw new Error("relay_ID doit être un nombre valide");
    }
...
exports.getEtatRelais = async () => {
  try {
    console.log(
      `➡️ Récupération de l'état des relais depuis : ${RASPBERRY_PI_URL}/relays-state`
    );
    ...
    throw new Error(
      "Impossible de récupérer l'état des relais sur le Raspberry Pi"
    );
  }
};
