...
const demarrerAssignationRFID = async (req, res) => {
  try {
    ...
    if (utilisateur.cardId) {
      return res.status(400).json({
        success: false,
        message: 'Cet utilisateur a déjà une carte RFID assignée. Désassignez-la d\'abord.'
      });
    }
    
    res.status(200).json({
      success: true,
      message: 'Mode d\'assignation RFID démarré. En attente de présentation d\'une carte.'
    });
  } catch (error) {
...
const assignerEmpreinte = async (req, res) => {
  try {
    ...
    if (utilisateur.empreinteID) {
      return res.status(400).json({
        success: false,
        message: 'Cet utilisateur a déjà une empreinte digitale assignée. Désassignez-la d\'abord.'
      });
    }
    
    // Déclencher l'enregistrement de l'empreinte
    const result = await fingerprintService.declencherEnregistrement(utilisateurId);
    ...
    res.status(500).json({
      success: false,
      message: 'Erreur lors de l\'assignation de l\'empreinte digitale: ' + error.message
    });
  }
};
  ...
  try {
    ...
    if (!fingerprintService.isDeviceConnected()) {
      ...
      return res.status(200).json({
        success: true,
        message: 'Empreinte digitale désassignée avec succès dans la base de données. Dispositif non connecté.',
        data: utilisateur
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: 'Erreur lors de la désassignation de l\'empreinte digitale: ' + error.message
    });
  }
};
  ...
  try {
    ...
    if (!statut) {
      return res.status(200).json({
        success: true,
        statut: 'aucun',
        message: 'Aucun enregistrement d\'empreinte en cours pour cet utilisateur',
    ...
    res.status(500).json({
      success: false,
      message: 'Erreur lors de la vérification du statut de l\'empreinte: ' + error.message
    });
  }
};
