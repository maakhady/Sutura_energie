...
//         console.log("Données semaine reçues:", response);

//         if (response && response.data) {
//           const formattedData = formatData(response.data);
//           console.log("Données semaine formatées:", formattedData);
//           setConsommationData(formattedData);
//         } else {
//           console.warn("Aucune donnée de semaine reçue");
//           setConsommationData([]);
//         }
//       } else {
//         response = await EnergieService.getConsommationMois();
//         console.log("Données mois reçues:", response);

//         if (response && response.data) {
//           const formattedData = formatMonthlyData(response.data);
//           console.log("Données mois formatées:", formattedData);
//           setConsommationData(formattedData);
//         } else {
//           console.warn("Aucune donnée de mois reçue");
//           setConsommationData([]);
//         }
//       }
//     } catch (error) {
...
//   // Effet pour le chargement initial des données utilisateur et logs
//   useEffect(() => {
...
//   // Effet pour le chargement initial des données de consommation
//   useEffect(() => {
//     // Charge les données lors du premier rendu
...
//                     <p>Aucune donnée de consommation disponible pour la période sélectionnée.</p>
//                     <button 
//                       className="reload-btn" 
//                       onClick={fetchConsommationData}
//                     >
...
import { EnergieService } from "../services/EnergieService";
import { socket, getEmergencyStatus } from "../utils/socket";
import speechService from "../services/SpeechService"; // Importer le service de synthèse vocale
import Swal from "sweetalert2";
import {
  FileUpIcon,
  LineChartIcon,
...
const DashboardHistorique = () => {
  ...
  useEffect(() => {
    ...
    if (currentEmergency) {
      ...
      // Annonce vocale de l'alerte - conservée car importante pour la sécurité
      speechService.speak("Alerte de sécurité ! " + currentEmergency.message);
    }
    ...
    const handleEmergency = (data) => {
      ...
      // Annonce vocale de l'alerte - conservée car importante pour la sécurité
      speechService.speak("Alerte de sécurité ! " + data.message);
    };
  ...
  const nextPage = () => {
    if (currentPage < totalPages) {
      setCurrentPage(currentPage + 1);
      // Annonce vocale de la nouvelle page - conservée car interactive
      const message = speechService.phrases.LOGS_PAGE.replace("{0}", currentPage + 1);
    ...
    try {
      ...
      if (selectedPeriod === "semaine") {
        ...
        if (response && response.data) {
          ...
          // Synthèse vocale des données chargées SUPPRIMÉE
        } else {
          console.warn("Aucune donnée de semaine reçue");
          setConsommationData([]);
          // Synthèse vocale de l'absence de données SUPPRIMÉE
        }
      } else {
        response = await EnergieService.getConsommationMois();
        console.log("Données mois reçues:", response);
      ...
      // Synthèse vocale de l'erreur SUPPRIMÉE
    } finally {
      setLoadingConsommation(false);
    }
  };
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        if (response && response.success) {
          setUtilisateur(response.data);
          // Message de bienvenue vocal SUPPRIMÉ
        }
      } catch (error) {
        console.error("Erreur lors de la récupération du profil:", error);
      } finally {
            ...
            <div className="period-selector">
              <select
                ...
                onChange={(e) => {
                  ...
                  // Annonce vocale du changement de période - conservée car interactive
                  if (newPeriod === "semaine") {
                    speechService.speak(speechService.phrases.CONSOMMATION_SEMAINE_AFFICHAGE);
                  } else {
                    speechService.speak(speechService.phrases.CONSOMMATION_MOIS_AFFICHAGE);
                  }
                }}
                className="period-dropdown"
              >
            ...
            <div className="tabs">
              <button
                ...
                onClick={() => {
                  ...
                  speechService.speak("Affichage de la consommation énergétique");
                }}
              >
                <LineChartIcon size={20} />
                Consommation
              ...
              <button
                ...
                onClick={() => {
                  setActiveTab("logs");
                  speechService.speak(speechService.phrases.LOGS_AFFICHAGE);
                }}
              >
                <LogsIcon size={20} />
                Logs d{"'"}activité
                ...
                {loadingConsommation ? (
                  ...
                  <div className="no-data-message">
                    ...
                    <button 
                      ...
                      onClick={() => {
                        fetchConsommationData();
                        // Synthèse vocale pour le rechargement des données SUPPRIMÉE
                      }}
                    >
                      Actualiser les données
                    </button>
              ...
              <div className="activity-logs">
                <div className="logs-header">
                  ...
                  <button 
                    ...
                    onClick={() => {
                      // Synthèse vocale pour l'exportation - conservée car interactive
                      speechService.speak("Exportation des logs d'activité");
                    }}
                  >
                    <FileUpIcon size={20} />
                    Exporter
